...
1. `prisma` 会在 SQL 中生成一个日志表，会对日志生成时间有较为严格的对比，这就导致在多人开发过程中，对表的创建和修改产生的更新日志时间无法保证统一的顺序，导致的结果就是在更新表的时候，会提示表错误，然后让其选择是否进行数据库格式化 --(egg 疼)
2. 这里我采用的方法是，测试环境想提交就提交。`npx prisma db push`，其他人在开发环境的时候，进行 `pull` 就可以了。
3. 生产环境时，则通过命令将生产环境表与测试环境表进行 `diff` 对比，然后生成修改文件。手动保留需要上线的表修改并生成日志文件进行同步。
4. 生产环境执行日志更新，来达到表更新的目的。
5. 涉及到的两个文件：`prisma-diff.sh` 这个会对比生产表与测试表的不同并生成修改文件。`prisma-up` 这里会将生成的修改文件打包成修改迁移日志。发布线上后，线上执行 `npx prisma migrate deploy` 来同步日志。
